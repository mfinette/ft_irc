...
void	Command::PRIVMSG(Client &client){
	...
	//	ERR_CANNOTSENDTOCHAN(client, "thischannel"); //si le msg n'as pas pu etre envoyer car pas les droits (ou autre)
		return;
	}
	...
	//si la target commence avec "$" le msg doit etre envoyer a tous les clients du serveur
	if (msg.size() == 0)
		return ERR_NOTEXTTOSEND(client);
